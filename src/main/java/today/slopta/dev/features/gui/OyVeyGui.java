...
public class OyVeyGui extends Screen {
    ...
    private void load() {
        int x = -84;
        for (final today.slopta.dev.features.modules.Module.Category category : Slopta.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Slopta.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
        ...
        this.components.forEach(components -> components.getItems().sort(Comparator.comparing(Module::getName)));
    }

    public void updateModule(today.slopta.dev.features.modules.Module module) {
        for (Component component : this.components) {
            for (Item item : component.getItems()) {
                if (!(item instanceof ModuleButton)) continue;
                ModuleButton button = (ModuleButton) item;
                today.slopta.dev.features.modules.Module mod = button.getModule();
