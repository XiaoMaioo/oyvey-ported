...
import today.slopta.dev.util.ColorUtil;
...
public class BindButton
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515) : (!this.isHovering(mouseX, mouseY) ? Slopta.colorManager.getColorWithAlpha(Slopta.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Slopta.colorManager.getColorWithAlpha(Slopta.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue())));
        if (this.isListening) {
