...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Slopta.colorManager.getColorWithAlpha(Slopta.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Slopta.colorManager.getColorWithAlpha(Slopta.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
